...
class Manager extends FlxBasic
{
	...
	private var __triRenderQueue:Array<Dynamic> = [];
	...
	public function getHoldQuads(basePos:Vector3D, params:NoteData):Array<Dynamic>
	{
		...
		{
			var visuals = i == 0 ? output1.visuals : output2.visuals;
	...
	function drawHoldArrow(arrow:Note, holdScale:Float = 1) @:privateAccess {
		...
		for (camera in cameras)
			camera.drawTriangles(arrow.graphic, _vertices, _indices, _uvtData, _colors, null, arrow.blend, false, false, arrow.colorTransform, arrow.shader);
	}
	// TODO: Optimize this
	/**
	...
	private function __drawArrow(pos:Vector3D, output:ModifierOutput, sprite:FlxSprite, lane:Int, field:Int)
	{
		...
		for (camera in cameras)
			camera.drawTriangles(sprite.graphic, vertices, new Vector<Int>(vertices.length, true, [for (i in 0...vertices.length) i]), uvData, new Vector<Int>(), null, sprite.blend, false, false, sprite.colorTransform, sprite.shader);
	}
